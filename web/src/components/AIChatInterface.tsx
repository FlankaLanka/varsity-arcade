...
import { Send, Bot, Sparkles, Trash2, Image } from 'lucide-react';
...
const getWelcomeMessage = (members: Array<{ username: string }>, currentProblem?: CohortProblem | null): AIChatMessage => {
  const memberNames = members.length > 0 
    ? members.map(m => m.username).join(', ')
    : 'everyone';
  
  let content = members.length > 0
    ? `Hello ${memberNames}! I'm your AI Tutor. I'm here to help guide you through problems together.`
    : "Hello! I'm your AI Tutor. I can help guide you through problems.";
  
  if (currentProblem) {
    content += ` Let's work on today's challenge: "${currentProblem.question}" - What do you think would be a good first step?`;
  } else {
    content += " This is an open canvas - feel free to draw, collaborate, and explore! I'm here to help with any questions you have.";
  }
...
export default function AIChatInterface({ cohortId, whiteboardSnapshot, members = [], currentProblem }: AIChatInterfaceProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(chatRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (firebaseMessages.length === 0 && !hasInitializedRef.current) {
          const welcomeMsg = getWelcomeMessage(members, currentProblem);
          setMessages([welcomeMsg]);
          // Send welcome message to Firebase
        ...
        // No messages in Firebase, show welcome message
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      if (messagesToSend.length === 0) {
        messagesToSend = [getWelcomeMessage(members, currentProblem)];
      }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-900 min-h-0">
      ...
      {/* Action Buttons */}
      <div className="px-3 py-2 flex items-center justify-between gap-2 border-t border-gray-800 shrink-0">
        <button
          onClick={handleClearChat}
          className="flex items-center justify-center px-2.5 py-1.5 text-xs font-medium bg-red-900/20 border border-red-500/50 text-red-400 rounded hover:bg-red-900/30 transition-colors"
          title="Clear all messages and AI memory"
        >
          <Trash2 size={14} />
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={whiteboardSnapshot ? "Include whiteboard content in AI requests" : "Whiteboard not available"}
          >
            <span
          ...
          <button
            onClick={handleAskAI}
            disabled={isAskingAI || messages.length === 0}
            ...
            title="Ask AI Tutor for help with the conversation"
          >
            <Sparkles size={14} />
            ASK AI
          </button>
      ...
      {/* Input Area */}
      <form onSubmit={handleSendMessage} className="p-4 bg-gray-900 border-t border-gray-800 shrink-0">
        <div className="relative">
          <input
            ...
            className="w-full bg-black/50 border-2 border-gray-700 rounded-lg py-3 pl-4 pr-12 text-sm text-white focus:outline-none focus:border-neon-green focus:shadow-[0_0_10px_rgba(41,255,100,0.2)] transition-all"
          />
          <button
            type="submit"
            disabled={!inputValue.trim()}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-neon-green disabled:opacity-50 disabled:hover:text-gray-400 transition-colors"
          >
            <Send size={20} />
          </button>
        </div>
